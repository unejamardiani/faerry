...
fn includes_claude_mcp_preview(action: &str) -> bool {
    matches!(
        action,
        "dryRunMcps" | "syncMcps" | "dryRunAll" | "syncAll" | "syncClaudeCode"
    )
}
...
fn includes_codex_mcp_preview(action: &str) -> bool {
    matches!(
        action,
        "dryRunMcps" | "syncMcps" | "dryRunAll" | "syncAll" | "syncCodex"
    )
}
...
fn includes_opencode_mcp_preview(action: &str) -> bool {
    matches!(
        action,
        "dryRunMcps" | "syncMcps" | "dryRunAll" | "syncAll" | "syncOpenCode"
    )
}
...
fn link_preview_sections(repo: &AgentsRepo) -> Vec<DiffSection> {
    ...
    sections.push(text_diff_section(
...
fn symlink_section(title: &str, target: String, expected: String) -> DiffSection {
    ...
    let status = if current == expected_text {
        "unchanged"
    } else {
        "changed"
    };
...
fn codex_mcp_diff_section(
    ...
    let next = format!(
        "{}{}",
        remove_managed_toml(&current, &managed_names),
        codex_toml_block(servers)
    );
    Ok(text_diff_section(
        "Codex MCP config",
        &path,
        &current,
        &next,
...
fn codex_toml_block(servers: &[RegistryServer]) -> String {
    let mut lines = vec![
        ...
        "# Source: ~/.agents/mcp/servers.json".into(),
    ];
...
fn opencode_mcp_diff_section(
    ...
    let mut config: Value = if current.trim().is_empty() {
        ...
        serde_json::from_str(&strip_json_comments(&current))
            .map_err(|error| format!("OpenCode config is invalid JSON: {error}"))?
    };
    ...
    for server in servers
        .iter()
        .filter(|server| target_enabled(server, "opencode"))
    {
